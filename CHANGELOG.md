...
### Fixed
* Removed older products from s3 and updated 
* Updated delivery script so that we use .env file rather than version.
...
### Fixed
* Dockerfile using pixi base was not running correctly. Needed straightforward updates to welcome pixi usage including ignoring .pixi/ directory, ensuring .pixi directory had correct permissions, virtual environment activation slightly different than conda, etc.
...
### Fixed
* Sensor validation in product naming validation was not allowing for sensor Sentinel-1D (data_models/output_models.py)
